...
const RETRY_ERRORS: RetryErrors<Box<dyn Error>> =
    Retry(2).on_error(|err| err.to_string().contains("retry"));
...
#[test]
#[decorate(Retry(3), ShouldError("oops"))]
...
#[async_std::test]
#[decorate(Retry(2), Timeout(Duration::from_millis(100)))]
